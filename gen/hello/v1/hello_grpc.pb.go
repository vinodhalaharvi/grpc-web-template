...
const (
	...
	GreeterService_SayHelloStream_FullMethodName = "/hello.v1.GreeterService/SayHelloStream"
)
...
type GreeterServiceClient interface {
	...
	// Stream hellos (server streaming)
	SayHelloStream(ctx context.Context, in *SayHelloRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[SayHelloResponse], error)
}
...
func (c *greeterServiceClient) SayHelloStream(ctx context.Context, in *SayHelloRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[SayHelloResponse], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
...
type GreeterService_SayHelloStreamClient = grpc.ServerStreamingClient[SayHelloResponse]
...
type GreeterServiceServer interface {
	...
	// Stream hellos (server streaming)
	SayHelloStream(*SayHelloRequest, grpc.ServerStreamingServer[SayHelloResponse]) error
...
func (UnimplementedGreeterServiceServer) SayHelloStream(*SayHelloRequest, grpc.ServerStreamingServer[SayHelloResponse]) error {
...
func _GreeterService_SayHelloStream_Handler(srv interface{}, stream grpc.ServerStream) error {
	...
	return srv.(GreeterServiceServer).SayHelloStream(m, &grpc.GenericServerStream[SayHelloRequest, SayHelloResponse]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type GreeterService_SayHelloStreamServer = grpc.ServerStreamingServer[SayHelloResponse]
...
var GreeterService_ServiceDesc = grpc.ServiceDesc{
	ServiceName: "hello.v1.GreeterService",
